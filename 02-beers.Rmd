...
When you are first learning to program, one of the best strategies you can do is to copy working code and make small changes to the code to produce a different result.  In the `plot1` chunk (remember to look at the phrase right after the `{r` and before the comma that begins your R chunk to find the name), you will see how to produce a scatterplot of `Dependent1` on the vertical axis and `Independent1` on the horizontal axis (frequently written as "a scatterplot of `Dependent1` versus `Independent1`---or `y` versus `x`).  In the GIF below, you'll see how we could produce a plot of "log base 10" `Dependent1` versus `Independent1`.  Notice that only a few subtle changes are needed to produce a different plot.  Also think about how the new plot in chunk `plot2` compares to the plot produced in `plot1`.
