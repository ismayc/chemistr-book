...
When you are first learning to program, one of the best strategies you can do is to copy working code and make small changes to the code to produce a different result.  In the `plot1` chunk (remember to look at the phrase right after the `{r` and before the comma that begins your R chunk to find the name), you were shown how to produce a scatterplot of `Dependent1` on the vertical axis and `Independent1` on the horizontal axis (frequently written as "a scatterplot of `Dependent1` versus `Independent1`).  In the GIF below, you'll see how we could produce a plot of "log base 10" `Dependent1` versus `Independent1`.  (Remember that you first need to create the log variable.)
